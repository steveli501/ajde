...
class Url {
	...
	public static function read($url) {
		switch (self::getMode()) {
			...
			case self::MODE_NONE:
			default:
				throw new Exception(sprintf(self::$_errMessage, "Url::read()"));
				return false;
		}
	}
	...
	public static function getContents($url) {
		switch (self::getMode()) {
			...
			case self::MODE_NONE:
			default:
				throw new Exception(sprintf(self::$_errMessage, "Url::getContents()"));
				return false;
		}
...
if (URL_USE_GLOBAL_FUNCTIONS) {
	 ...
	 * @return mixed Returns the number of bytes read from the file. If an error occurs, FALSE is returned and unless the function was called as @readurl(), an error message is printed.
	 */
	function readurl($url) { return Url::read($url); }
	 ...
	 * @return mixed The function returns the read data or FALSE on failure.
	 */
	function url_get_contents($url) { return Url::getContents($url); }
