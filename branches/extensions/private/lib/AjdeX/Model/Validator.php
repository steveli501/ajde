...
class AjdeX_Model_Validator extends Ajde_Object_Standard
{	
	/**
	 *
	 * @var AjdeX_Model
	...
	public function __construct(AjdeX_Model $model)
	{
		$this->_model = $model;
	}
		...
		foreach($fieldOptions as $fieldName => $fieldProperties) {
			switch ($fieldProperties['type']) {
				case AjdeX_Db::FIELD_TYPE_DATE:				
					$this->_model->addValidator($fieldName, new AjdeX_Model_Validator_Date());
					break;
				case AjdeX_Db::FIELD_TYPE_NUMERIC:			
					$this->_model->addValidator($fieldName, new AjdeX_Model_Validator_Numeric());
					break;
				case AjdeX_Db::FIELD_TYPE_TEXT:
					$this->_model->addValidator($fieldName, new AjdeX_Model_Validator_Text());
					break;
				case AjdeX_Db::FIELD_TYPE_ENUM:
					$this->_model->addValidator($fieldName, new AjdeX_Model_Validator_Enum());
					break;
				case AjdeX_Db::FIELD_TYPE_SPATIAL:
					$this->_model->addValidator($fieldName, new AjdeX_Model_Validator_Spatial());
					break;
				default :
					break;
			}
			
			if ($fieldProperties['isRequired'] === true) {
	...
	public function validate($options = array())
	{
		...
		foreach($this->_model->getValidators() as $fieldName => $fieldValidators) {
			foreach($fieldValidators as $fieldValidator) {
				/* @var $validator AjdeX_Model_ValidatorAbstract */
				$value = null;
